...
class ProsetTest(AsyncTest):
    async def main(self):
        with AccountContext('admin@test', 'testtest') as admin_session:
            ...
            html = self.get_html('proset?show=onlyac', admin_session)
            self.assertEqual(len(html.select('#prolist > tbody > tr')), 2)

            html = self.get_html('proset?show=notac', admin_session)
            self.assertEqual(len(html.select('#prolist > tbody > tr')), 1)

            html = self.get_html('proset?order=chal&reverse=True', admin_session)
            trs = html.select('#prolist > tbody > tr')
