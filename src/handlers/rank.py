...
class ProRankHandler(RequestHandler):
    ...
    async def get(self, pro_id: int = None):
        ...
        err, result = await RankService.inst.get_pro_rank(pro_id, pageoff, pagenum)
        if err:
            return self.error(err)
        chal_list, total_cnt = result
...
class UserRankHandler(RequestHandler):
    ...
    async def get(self):
        ...
        err, result = await RankService.inst.get_user_rank(pageoff, pagenum)
        if err:
            return self.error(err)
        acctlist, total_cnt = result
