                    ...
                    if include_system_test:
                        ...
                        while True:
                            ...
                            for subtask in newly_invalid:
                                ...
                                need_judge_testdatas.difference_update(
                                    subtask['testdatas']
                                )
                                await ChalService.inst.update_subtask_result(
                                    chal_id,
                                    SubtaskResult(subtask_id, ChalConst.STATE_JE, 0, 0, decimal.Decimal())
                                )
                        ...
                        subtasks = [
                            subtask
                            for subtask in subtasks
                            if subtask['id'] not in invalid_subtasks
                        ]
        ...
        try:
            async with db.acquire() as con:
                async with con.transaction():
                    ...
                    try:
                        with open(f"code/{chal_id}/main.{source_ext}", 'wb') as code_f:  # pragma: no branch
